...
export class AuthMiddleWare implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction): void {
    ...
    console.log({
      includes: req.body?.query?.includes('mutation RefreshTokens'),
    });

    if (
      req.body?.query?.includes('register') ||
      req.body?.query?.includes('signout')
    ) {
      // Perform actions specific to the isolated mutation
      console.log('Intercepted register mutation. No need for a token here ');
      next();
    } else if (req.body?.query?.includes('mutation RefreshTokens')) {
      console.log('refreshTokens mutation else if');
      try {
        const decodedRefreshToken = this.jwtService.verify(refreshtoken, {
          complete: true,
          ignoreExpiration: false,
        ...
        console.log({ error });
        console.log('sending back 401 response...');
        throw new Error('jwt tokens expired');
      }
      ...
      try {
        ...
        console.log('inside  catch statement');
        console.log("access token verification failed because it's expired");
        console.log('sending error 401 back');
        res.sendStatus(401);
      }
    }
  }
